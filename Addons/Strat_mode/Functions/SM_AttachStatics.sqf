STATICS_MG = ["rhs_KORD_high_MSV","RHS_M2StaticMG_WD"];
STATICS_LMG = ["RHS_AGS30_TriPod_MSV","RHS_MK19_TriPod_WD","RHS_NSV_TriPod_MSV","RHS_M2StaticMG_MiniTripod_WD"];
STATICS_L = ["rhs_SPG9M_MSV","rhs_Kornet_9M133_2_msv","RHS_TOW_TriPod_WD"];
STATICS_A = ["rhs_D30_msv","rhs_D30_at_msv","RHS_M119"];
...
Attach_Static={
  ...
  if (!(_veh iskindOf "rhs_kamaz5350_flatbed_msv" || _veh iskindOf "rhsusf_M1084A1P2_WD_fmtv_usarmy")) then {
  ...
  if (_veh iskindOf "rhs_kamaz5350_flatbed_msv") then {
    ...
    if((_NO select 0) isKindOf "RHS_M119") then {[(_NO select 0),1] call rhs_fnc_m119_fold; (_NO select 0) attachto [_veh, [0,0.3,-1]];};
    if((_NO select 0) isKindOf (STATICS_M select 0)) then {(_NO select 0) attachto [_veh, [0,0.8,0]];};
  };
  if (_veh iskindOf "rhsusf_M1084A1P2_WD_fmtv_usarmy") then {
    ...
    if((_NO select 0) isKindOf "RHS_M119") then {[(_NO select 0),1] call rhs_fnc_m119_fold; (_NO select 0) attachto [_veh, [-0.1,0.5,-0.4]];};
